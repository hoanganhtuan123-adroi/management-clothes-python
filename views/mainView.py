...
class MainView(tk.Toplevel):
    def __init__(self, master=None):
        ...
        for F in (KhachHangFrame, SanPhamFrame, DonHangFrame, KhoHangFrame, NhanVienFrame, NhaCungCapFrame, BaoCaoFrame, PhanQuyenFrame):
            page_name = F.__name__
            frame = F(self.main_container)
            self.frames[page_name] = frame
            # Dùng grid để chồng các frame lên nhau
            frame.grid(row=0, column=0, sticky="nsew")

        # Hiển thị frame mặc định ban đầu
